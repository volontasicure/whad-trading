...
9. **Filtro di trend su VWAP reversion, costruito il 17/9 ma NON deployato** — in risposta al 16/9 (VWAP −$854 lab / parte di −$1.818 sul conto reale, 5 rientri consecutivi su BA in mean-reversion contro un ribasso vero). Aggiunge `TrendContext` (SMA 20gg + Efficiency Ratio di Kaufman, `server/technicalIndicators.ts`) come filtro opzionale in `vwapReversion.ts` (`decideEntries`), già threaded in `tick.ts`/`realExecution.ts`/`scripts/backtest.ts`. Backtest A/B: sul 16/9 specifico −605 → −46 (quasi risolto), sul periodo recente aggregato marginale (+4.057 → +4.148), ma **peggiora fuori campione** su un periodo precedente (−2.382 → −2.678) — stesso pattern di overfitting del punto 10. Codice committato in locale (`024a623`, "Add VWAP trend filter... — NOT pushed, held back"), **non pushato**: produzione gira ancora senza filtro. Promemoria per riparlarne con più dati reali: routine cloud `trig_01Lm7gcEEpkcBoCT1TFUKDjS`, 21/9.
